...
client.on("guildMemberAdd", async member => {
    ...
    ctx.fillText("Bienvenue 👋", canvas.width / 2.2, canvas.height / 3)
    ...
    let setrole = member.guild.roles.cache.find(role => role.name === '🤜BG🤛')
    user.roles.add(setrole).catch(console.error)
...
client.on('guildMemberRemove', async member => {
    ...
    ctx.fillText(user.user.username, canvas.width / 2.2, canvas.height / 1.7);
    ctx.fillText("Au revoir 😢", canvas.width / 2.2, canvas.height / 3)
